...
4. **暗号化デモ資産フォルダ** `ITA_Track1_HackTheLiquidWAY_DemoAssets`
...
- [x] 正式 **TEAM NAME = ITA** に決定（各ファイル/フォルダ名・スライドへ反映済み）。
- [x] **公開リポジトリ push 済み** → https://github.com/kous1001/tomoshibi
...
- [x] **公開GitHubリポジトリ作成→push→URL取得** → https://github.com/kous1001/tomoshibi
   - [x] URL を スライド4・`DEMO_ASSETS_README.txt`・`TEAM_BIOS.md` に反映済み。
   - [ ] 以後コード/ドキュメント更新時は `git add -A && git commit && git push` で同期。
- [ ] HTMLスライドを **PDF 書き出し**（ブラウザで開く→Cmd+P→Landscape→Save as PDF）。
- [ ] デモ資産を**暗号化**（zip暗号化等）し、フォルダ名 `ITA_Track1_HackTheLiquidWAY_DemoAssets`。
